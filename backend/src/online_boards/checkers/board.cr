...
module Checkers
  class Board
    ...
    def legal_and_empty_square?(square)
      self.legal_square?(square) && self.empty_square?(square)
    end

    def legal_and_opponent_square?(square, piece)
      other_piece = self.piece_at(square)
      self.legal_square?(square) && other_piece && piece.opponent?(other_piece)
    end
    ...
    def apply_move(move)
      ...
      if piece
        if self.promotion_square?(move.end_square, piece)
          piece.promote
        end
    ...
    def moves(color)
      ...
      if captures.empty?
        ...
        captures.select do |move| move.captures.size == captures_sizes_max end
      end
    end
      ...
      while !explore.empty?
        ...
        {-1, 1}.each do |rank_dir|
          {-1, 1}.each do |file_dir|
            ...
            if piece.king?
              while self.legal_and_empty_square?(capture_square)
                capture_square = capture_square.add(rank_dir, file_dir)
              end
            end
            if self.legal_and_opponent_square?(capture_square, piece)
              if !move.captures.includes?(capture_square)
                end_square = capture_square.add(rank_dir, file_dir)
                while self.legal_and_empty_square?(end_square)
                  move = move.clone()
    ...
    def piece_steps(piece, square)
      ...
      if piece.man?
        self.man_steps(piece, square, end_squares)
      else
        self.king_steps(piece, square, end_squares)
      end

      end_squares.map do |end_square| Move.new_step(square, end_square) end
    end
    ...
    def man_steps(piece, square, end_squares)
      ...
      if self.legal_and_empty_square?(left_step)
        end_squares << left_step
      end

      if self.legal_and_empty_square?(right_step)
